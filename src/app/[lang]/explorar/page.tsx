...
export async function generateMetadata({
  ...
  return buildPageMetadata({
    title: t(locale, "explorar.metadata.title"),
    description: t(locale, "explorar.metadata.description"),
    pathname: "/explorar",
    locale,
  });
}
...
export default async function ExplorarPage({
  ...
  const savedSearchHref = `${localizedHref(lang, "/explorar")}?${encodeFilters(filters)}`;
  const savedSearchLabel = [
    ...
    .filter(Boolean)
    .slice(0, 3)
    .join(" · ") || t(lang, "explorar.savedSearch.default");
